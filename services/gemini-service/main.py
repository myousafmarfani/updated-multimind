...
SERVICE_ID = "gemini"
MODEL_NAME = os.getenv("cognitivecomputations/dolphin-mistral-24b-venice-edition:free")
...
load_env_file()
API_KEY = os.getenv("OPENROUTER_API_KEY_03", "")
...
app = FastAPI(title="Gemini Service", version="1.0.0")
...
def call_openrouter(payload: ChatRequest) -> str:
    ...
    response_text = extract_content_text(content)
...
def synthesize_fallback_response(payload: ChatRequest, reason: str) -> str:
    instruction_line = f"Instructions: {payload.instructions}\n" if payload.instructions else ""
    context_count = len(payload.conversationHistory)

    return (
        "OpenRouter could not return a live response right now.\n"
        f"Reason: {reason}\n\n"
        f"{instruction_line}"
        f"Conversation turns used: {context_count}.\n"
        f"Prompt summary: {payload.prompt[:1000]}"
    )
